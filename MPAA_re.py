...
자료 = {}
for 자산 in 자산군:
    자료[자산] = 파일.parse(sheet_name=자산, index_col='날짜', parse_dates=True)
    print(자산 + '개수', len(자료[자산].columns))
...
def 모멘텀순위(데이터, 순위):
    x = 평균모멘텀(데이터)
    # y = x.iloc[:, 0: len(x.columns)].rank(axis=1, ascending=0)
    y = x.rank(axis=1, ascending=0)
    y[y <= 순위] = 1
    y[y > 순위] = 0
    return y
